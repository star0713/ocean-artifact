...
app.get('/random/:sketchid', async (req, res) => {
    ...
    if (sketchid > 0 && sketchid < 10) {
        const filePath = path.resolve(__dirname + "/lib/prototype.html");
        const p5Script = fs.readFileSync(__dirname + '/dist/browse-' + sketchid + '.js');
        const sketchScript = fs.readFileSync(__dirname + '/dist/p5.min.js');
        const prototype = fs.readFileSync(__dirname + '/src/api/prototype.html', 'utf8');
